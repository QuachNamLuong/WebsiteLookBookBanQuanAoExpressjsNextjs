...
export async function validatePostSlugHelper(prisma: PrismaClient, postId: number, slug: string, postCategoryId: number) {
    try {
        ...
        if (post && post.id !== postId) throw new AppError(
            1000,
            "Slug already extist",
            "Slug already extist",
